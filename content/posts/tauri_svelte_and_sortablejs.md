...
applications; this adds complexity beyond what you would expect in a web app. Before getting started, I searched for resources that would tell
me exactly how to integrate all three things together. Sadly, there were none.
...
From the Svelte side, everything looks great now. In fact, it will work perfectly if you throw it in a REPL,
which is maddening (yes, that's how I first tried to debug)! The problem I had was that I could not drag any of the items in the list
in my app.

This is the first Tauri-introduced issue I encountered. The OS (i.e. Mac) drag-and-drop functionality was preventing the drag-and-drop action 
from registering in the webview. To avoid this, I had to add `dragDropEnabled: false` to my `tauri.conf.json`:
```json
{
    "app": {
      "windows": [
...
like a race condition along the lines of the list being rendered before the sortable functionality is attached. However, that hypothesis
...
I noticed that the original event is actually a `DragEvent`, which seems a little different from a pointer-based event (not that I know any
meaningful differences, did I mention I'm not a frontend developer?). I responded to Amp with this:
...
research. Amp reported that "HTML5 Drag and Drop has known quirks, especially in Electron-like environments like Tauri", and when
...
docs carefully enough to notice the `forceFallback` parameter, especially since it wasn't an option I was looking for. On the other hand, AI
